export default {
  ...
  admin: {
    ...
    accounts: {
      ...
      accountRuleManager: '账号规则',
      openAIAutoDisableRules: 'OAI 自动禁用',
      openAIAutoDisableRulesTitle: 'OpenAI 自动禁用规则',
      openAIAutoDisableRulesEnabled: '启用 OpenAI 自动禁用规则',
      openAIAutoDisableRulesHint: '当 OpenAI 上游响应命中指定状态码，或响应体/message 包含关键词时，系统会自动禁用账号。',
      openAIAutoDisableRulesDisabledHint: '关闭后不会根据这些规则自动禁用账号，但规则会保留。',
      openAIAutoDisableRulesRules: '规则列表',
      openAIAutoDisableRulesEmpty: '当前还没有规则，点击右上角添加。',
      openAIAutoDisableRulesAdd: '添加规则',
      openAIAutoDisableRulesRemove: '删除规则',
      openAIAutoDisableRulesStatusCode: '状态码',
      openAIAutoDisableRulesStatusCodePlaceholder: '例如 401',
      openAIAutoDisableRulesKeywords: 'message 关键词',
      openAIAutoDisableRulesKeywordsPlaceholder: '例如 token_revoked, invalidated oauth token',
      openAIAutoDisableRulesKeywordsHint: '多个关键词可用逗号或换行分隔。状态码和关键词二选一即可，同时填写时命中任一条件都会自动禁用。',
      openAIAutoDisableRulesDescription: '备注',
      openAIAutoDisableRulesDescriptionPlaceholder: '可选，方便识别规则用途',
      openAIAutoDisableRulesLoadFailed: '加载 OpenAI 自动禁用规则失败',
      openAIAutoDisableRulesSave: '保存规则',
      openAIAutoDisableRulesSaving: '保存中...',
      openAIAutoDisableRulesSaved: 'OpenAI 自动禁用规则已保存',
      openAIAutoDisableRulesSaveFailed: '保存 OpenAI 自动禁用规则失败',
      openAIAutoDisableRulesInvalid: '请至少填写一个状态码或关键词',
      batchRefreshPendingOpenAI: '批量刷新待完善令牌',
      batchRefreshPendingOpenAIHint: '自动刷新所有未显示等级，或 Free 且未关闭训练共享的 OpenAI OAuth 账号',
      batchRefreshPendingOpenAIRefreshing: '批量刷新中...',
      batchRefreshPendingOpenAIConfirm: '将自动刷新所有未显示等级，或 Free 且未关闭训练共享的 OpenAI OAuth 账号，是否继续？',
      batchRefreshPendingOpenAINone: '当前没有需要刷新的 OpenAI OAuth 账号',
      batchRefreshPendingOpenAISuccess: '已成功刷新 {count} 个待完善 OpenAI OAuth 账号',
      batchRefreshPendingOpenAIPartial: '批量刷新部分完成：成功 {success} 个，失败 {failed} 个',
      batchRefreshPendingOpenAIFailed: '批量刷新待完善 OpenAI OAuth 账号失败',
      ...
      openAIAuthImport: '导入 auth.json',
      openAIAuthImportTitle: '批量导入 auth.json',
      openAIAuthImportHint: '支持通过文件上传或 JSON 数组接口批量导入 OpenAI OAuth 凭证。',
      openAIAuthImportFormatHint: '文件或接口负载必须是标准 JSON 数组，下面示例展示了单条记录结构。',
      openAIAuthImportWarning: '当前导入不会做去重，也不会自动绑定默认分组；每条记录都会创建一个新的 openai/oauth 账号，并且必须包含 refresh_token。',
      openAIAuthImportModeFile: '文件上传',
      openAIAuthImportModeJson: 'JSON 数组',
      openAIAuthImportFile: 'auth.json 文件',
      openAIAuthImportJson: 'JSON 内容',
      ...
      openAIAuthImportAccountOptionsTitle: '导入账号配置',
      openAIAuthImportAccountOptionsHint: '这些设置会统一应用到本次导入的所有 OpenAI OAuth 账号。',
      openAIAuthImportWSModeDefault: '不指定（沿用系统默认）',
      openAIAuthImportWSModeDefaultHint: '不指定时保留账号默认行为，只在这里显式选择时才写入 WS mode。',
      ...
      openAIAuthImportNameTemplatePresetDefault: '默认命名规则',
      openAIAuthImportNameTemplatePresetEmail: '仅邮箱：{email}',
      openAIAuthImportNameTemplatePresetPlanEmail: '套餐-邮箱：{plan_type}-{email}',
      ...
      openAIAuthImportSelectJson: '请输入 JSON 数组',
      openAIAuthImportMustBeArray: 'JSON 内容必须是数组',
      openAIAuthImportParseFailed: 'JSON 解析失败',
      openAIAuthImportFailed: 'auth.json 导入失败',
      openAIAuthImportResult: '导入结果',
      openAIAuthImportResultSummary: '账号创建 {account_created}，失败 {account_failed}',
      openAIAuthImportErrors: '失败详情',
      openAIAuthImportSuccess: '导入完成：账号创建 {account_created}，失败 {account_failed}',
      openAIAuthImportCompletedWithErrors: '导入完成但有错误：账号创建 {account_created}，失败 {account_failed}',
    ...
    usage: {
      ...
      ipAddress: 'IP',
      errorDetail: '错误详情',
      upstreamErrorDetail: '上游原始错误',
      userVisibleErrorBody: '返回给用户的报错',
    ...
    settings: {
      ...
      openAIRateLimitRecovery: {
        title: 'OpenAI 自动探测',
        description: '仅针对 OpenAI 平台。按固定间隔对选定状态的账号执行一次与账号管理“测试链接”相同的真实上游探测，探测结果会直接写回账号状态。',
        enabled: '启用自动探测',
        enabledHint: '开启后会按设定间隔对符合状态筛选的 OpenAI 账号执行一轮真实探测',
        testModel: '探测模型',
        testModelPlaceholder: '例如：gpt-5.1-codex',
        testModelHint: '所有 OpenAI 自动探测账号共用这一默认测试模型',
        testModelRequired: '请填写探测模型',
        checkIntervalMinutes: '探测间隔（分钟）',
        checkIntervalMinutesHint: '每隔多久对符合条件的 OpenAI 账号执行一轮探测，范围 1-1440 分钟',
        checkIntervalMinutesInvalid: '探测间隔必须在 1 到 1440 分钟之间',
        targetStatuses: '参与探测的账号状态',
        targetStatusesHint: '只有命中这些状态的 OpenAI 账号才会进入自动探测范围',
        targetStatusesRequired: '请至少选择一种参与探测的账号状态',
        autoRecover: '成功后自动恢复可恢复状态',
        autoRecoverHint: '开启后，探测成功会自动清理 error、限流、临时不可调度等可恢复运行时状态',
        saved: 'OpenAI 自动探测设置保存成功',
        saveFailed: '保存 OpenAI 自动探测设置失败'
      },
      rectifier: {
        title: '请求整流器',
