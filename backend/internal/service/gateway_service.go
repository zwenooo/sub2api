...
func (s *GatewayService) selectAccountForModelWithExclusions(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, bindSticky bool) (*Account, error) {
	...
	if (platform == PlatformAnthropic || platform == PlatformGemini) && !hasForcePlatform {
		account, err := s.selectAccountWithMixedSchedulingWithBinding(ctx, groupID, sessionHash, requestedModel, excludedIDs, platform, bindSticky)
		if err != nil {
			return nil, err
		}
	...
	account, err := s.selectAccountForModelWithPlatformWithBinding(ctx, groupID, sessionHash, requestedModel, excludedIDs, platform, bindSticky)
	if err != nil {
		return nil, err
	}
	return s.hydrateSelectedAccount(ctx, account)
